...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test 19: between()
    let q = QailCmd::get("harbors").column("id").filter_cond(between("id", 1, 5)).order_by("id", SortOrder::Asc);
    match driver.fetch_all(&q).await { Ok(r) => { passed += 1; println!("✅ between() - {} rows", r.len()); } Err(e) => { failed += 1; println!("❌ between(): {}", e); } }

    // Test 20: not_between()
    let q = QailCmd::get("harbors").column("id").filter_cond(not_between("id", 1, 5)).limit(5);
    ...
    // Test 21: is_in()
    let q = QailCmd::get("harbors").column("id").filter_cond(is_in("id", [1, 2, 3])).order_by("id", SortOrder::Asc);
    match driver.fetch_all(&q).await { Ok(r) => { passed += 1; println!("✅ is_in() - {} rows", r.len()); } Err(e) => { failed += 1; println!("❌ is_in(): {}", e); } }

    // Test 22: not_in()
    let q = QailCmd::get("harbors").column("id").filter_cond(not_in("id", [1, 2, 3])).limit(5);
    ...
    // Test 23: is_null()
    let q = QailCmd::get("harbors").column("id").filter_cond(is_null("name")).limit(3);
    match driver.fetch_all(&q).await { Ok(r) => { passed += 1; println!("✅ is_null() - {} rows", r.len()); } Err(e) => { failed += 1; println!("❌ is_null(): {}", e); } }

    // Test 24: is_not_null()
    let q = QailCmd::get("harbors").column("id").filter_cond(is_not_null("name")).limit(3);
    ...
    // Test 31: coalesce()
    let q = QailCmd::get("harbors").column_expr(coalesce([col("name"), text("N/A")]).alias("coalesced")).limit(1);
    match driver.fetch_all(&q).await { Ok(r) => { passed += 1; println!("✅ coalesce() - {} rows", r.len()); } Err(e) => { failed += 1; println!("❌ coalesce(): {}", e); } }

    // Test 32: concat()
    let q = QailCmd::get("harbors").column_expr(concat([col("name"), text(" - "), col("id").cast("text")]).alias("combined")).limit(1);
    ...
    // Test 33: now()
    let q = QailCmd::get("harbors").column_expr(now().with_alias("current_time")).limit(1);
    match driver.fetch_all(&q).await { Ok(r) => { passed += 1; println!("✅ now() - {} rows", r.len()); } Err(e) => { failed += 1; println!("❌ now(): {}", e); } }

    // Test 34: now_minus()
    let q = QailCmd::get("harbors").column_expr(now_minus("1 hour").with_alias("hour_ago")).limit(1);
    match driver.fetch_all(&q).await { Ok(r) => { passed += 1; println!("✅ now_minus() - {} rows", r.len()); } Err(e) => { failed += 1; println!("❌ now_minus(): {}", e); } }

    // Test 35: now_plus()
    let q = QailCmd::get("harbors").column_expr(now_plus("1 day").with_alias("tomorrow")).limit(1);
