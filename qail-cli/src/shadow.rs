...
impl ShadowState {
    pub fn new(primary_url: &str) -> Result<Self> {
        ...
        let shadow_url = if let Some(pwd) = &password {
            format!("postgres://{}:{}@{}:{}/{}", user, pwd, host, port, shadow_name)
        } else {
...
pub async fn create_shadow_database(primary_url: &str) -> Result<ShadowState> {
    ...
    println!("  {} Creating shadow database: {}", "[1/4]".cyan(), state.shadow_name.yellow());
    ...
    let mut admin_driver = if let Some(pwd) = password.clone() {
        PgDriver::connect_with_password(&host, port, &user, "postgres", &pwd).await
            .map_err(|e| anyhow!("Failed to connect to postgres: {}", e))?
    } else {
        PgDriver::connect(&host, port, &user, "postgres").await
            .map_err(|e| anyhow!("Failed to connect to postgres: {}", e))?
    };
    ...
    let existing = admin_driver.fetch_all(&check_cmd).await
    ...
    if !existing.is_empty() {
        ...
        admin_driver.execute_raw(&create_ddl).await
...
pub async fn apply_migrations_to_shadow(
    ...
    let mut shadow_driver = if let Some(pwd) = password {
        PgDriver::connect_with_password(&host, port, &user, &state.shadow_name, &pwd).await
            .map_err(|e| anyhow!("Failed to connect to shadow: {}", e))?
    } else {
        PgDriver::connect(&host, port, &user, &state.shadow_name).await
            .map_err(|e| anyhow!("Failed to connect to shadow: {}", e))?
    };
    
    for (i, cmd) in cmds.iter().enumerate() {
        shadow_driver.execute(cmd).await
...
pub async fn sync_data_to_shadow(state: &mut ShadowState) -> Result<()> {
    println!("  {} Syncing data from primary to shadow...", "[3/4]".cyan());
    ...
    let table_rows = primary_driver.fetch_all(&tables_cmd).await
    ...
    for table in &tables {
        ...
        let rows = primary_driver.fetch_all(&fetch_cmd).await
...
pub fn display_shadow_status(state: &ShadowState) {
    ...
    println!("    {} → Run tests against shadow", "qail shadow test".green());
    println!("    {} → Switch traffic to shadow", "qail shadow promote".green().bold());
    println!("    {} → Drop shadow, keep primary", "qail shadow abort".red());
    println!();
}
...
pub async fn promote_shadow(primary_url: &str) -> Result<()> {
    ...
    let old_name = format!("{}_old_{}", database, chrono::Utc::now().format("%Y%m%d%H%M%S"));
    
    println!("  [1/3] Renaming {} → {}", database.cyan(), old_name.yellow());
    let rename1 = format!("ALTER DATABASE {} RENAME TO {}", database, old_name);
    admin_driver.execute_raw(&rename1).await
        .map_err(|e| anyhow!("Failed to rename primary: {}", e))?;
    
    println!("  [2/3] Renaming {} → {}", state.shadow_name.yellow(), database.green());
    let rename2 = format!("ALTER DATABASE {} RENAME TO {}", state.shadow_name, database);
    admin_driver.execute_raw(&rename2).await
        .map_err(|e| anyhow!("Failed to promote shadow: {}", e))?;
    
    println!("  [3/3] Keeping old database as backup: {}", old_name.dimmed());
    
    println!();
    println!("{}", "✓ Shadow promoted successfully!".green().bold());
    println!("  Old database preserved as: {}", old_name.yellow());
    println!("  To clean up: {}", format!("DROP DATABASE {}", old_name).dimmed());
...
pub async fn abort_shadow(primary_url: &str) -> Result<()> {
    ...
    admin_driver.execute_raw(&drop_ddl).await
        .map_err(|e| anyhow!("Failed to drop shadow: {}", e))?;
    
    println!("{}", "✓ Shadow database dropped. Primary unchanged.".green());
